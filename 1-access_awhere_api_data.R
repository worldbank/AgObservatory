#--------------------------------------------------------------------------
# Ag Observatory R Tutorial: Access aWhere API Data 
#
# Purpose of script: 
...
# longitude of an area of interest by using Google Maps, QGIS, aWhere's 
...
# 1. A csv output of the Forecast (Hourly, 6 hour, 12-hour, or daily blocks 
#     of time).
# 2. Observed data for any time period between 2006 and present
# 3. Long-Term Normals (LTN, or Long-Term Average) for chosen time period 
...
# Next, create a text file (.txt) or comma separated value file (.csv) with this
# location information. It must have 3 columns called place_name, latitude,
# longitude.
# An example of a row with location information would be:
#     place_name, latitude, longitude
#     Nairobi, -1.283, 36.816

# CHANGE THIS to the path of your locations text or comma separated value file
locations_file <- "YOUR LOCATION FILE.txt" 
locations_file <- "YOUR LOCATION FILE.csv" 
...
for (i in(1:nrow(locations))) { 
  ...
  write.csv(forecast, file = paste0("outputCSVs/forecast-6hour-",place_name,
                                    ".csv"), row.names=F) 
  ...
  starting_date <- "2019-03-01" # October 1, 2019
  ...
  write.csv(observed, file = paste0("outputCSVs/observedData-",place_name,
                                    ".csv"), row.names=F) 
  ...
  # Here you will pull agronomic data for your location and time of interest. If
  # you do not change the "starting_date" and "ending_date" variables, then the
  # time period will remain the same from the observed data pulled above.
  ...
  write.csv(ag, file = paste0("outputCSVs/agronomicsData-",place_name,
                              ".csv"), row.names=F) 
  
  # Long Term Normals -------------------------------------------------------
  
  # Here you will pull the long-term normals (LTN, or Long-Term Average) for
  # your location and time period of interest.
  ...
  monthday_start <- "03-01" # March 1
  monthday_end <- "09-30"   # September 30
  ...
  write.csv(ltn, file = paste0("outputCSVs/ltnData-",place_name,
                               ".csv"), row.names=F) 
  ...
  starting_date <- "2019-03-01"
  ending_date <- "2019-09-30"
  year_start <- 2006
  year_end <- 2018
