#--------------------------------------------------------------------------
# Ag Observatory R Tutorial: Create Charts
#
# Purpose of script: 
#This code will show you how to quickly create a series of charts of your
#locations of interest to help you understand the weather/agronomic conditions
#in those locations based on aWhere's ag-weather dataset (includes variables
#such as temperature, precipitation, potential evapotranspiration, comparisons
#to long-term normals, and more). These charts can be added to reports for a
#quick analysis of the ag-weather conditions in your locations of interest. The
#weather data pulled for your location and time period of interest are also
#saved as a .csv file for future analysis.
#
#Prior to running this script, we encourage you to find the name, latitude, and
#longitude of your location(s) of interest by using Google Maps, QGIS and
#aWhere's geospatial files found on apps.awhere.com, or by using GPS points that
#you have previously collected.
#
#This script uses a locations file (.txt, .csv) which allows you to create the
#climatology chart for all of your locations of interest at once by using a
#process called a "loop." You will need to create a file with your location(s)
#of interest. You may edit the "locations.txt" text file sample in the RunSet
#folder. The locations file must have 3 columns called place_name, latitude,
#longitude: 
#     place_name, latitude, longitude
#     Harare, -17.81, 31.03
#     Nairobi, -1.28, 36.81
#     Addis Ababa, 8.98, 38.75
#
#Before continuing, please format your locations file (.txt, .csv).
#
...
# This script requires you to load a supporting functions file that is typically
# found in Github or saved in the Source folder in the recommended folder
# structure. This step loads additional functions required to create the
# climatology chart.
...
date_start <- "2019-09-15"  # CHANGE TO THE STARTING DATE OF YOUR CHOICE 
date_end <- as.character(Sys.Date()+14) # CHANGE TO THE ENDING DATE OF YOUR CHOICE 

# Long-term normal (LTN, or Long-Term Average) values will be calculated across
# this range of years.
year_start <- 2006  # Starting year can be as early as 2006
year_end <- 2018
...
for (i in 1:nrow(locations)) {
  ...
  lat_title <- round(lat, 3)
  lon_title <- round(lon, 3)
  
  # Create a variable containing both the lat and lon, for plot titles
  ...
  # Construct title
  max_temp_stdev_title <- paste0(place_name, ": Maximum Temperature")
  ...
  min_temp_stdev_title <- paste0(place_name, ": Minimum Temperature")
  ...
  pet_stdev_title <- paste0(place_name, ": PET")
  ...
  acc_pet_stdev_title <- paste0(place_name, ": Accumulated PET")
  ...
  rolling_avg_ppet_addyears_title <- 
    paste0(place_name
            ,": ",roll_window," day rolling average P/PET \n"
            ,"with additional selected years")
  ...
  rolling_avg_ppet_addyears <- rolling_avg_ppet + 
    geom_ribbon(data = add_years_rolling_avg_ppet
              ...
              ,size = line_width) +
    ggtitle(paste(rolling_avg_ppet_addyears_title,
                  lat_lon, "from", date_start, "to",date_end)) + 
    scale_list$colorScale +
    scale_list$fillScale + 
    xlim(as.Date(date_start), as.Date(date_end)) +
    guides(fill = guide_legend(ncol =  length(add_years) +2))
  ...
  if (save_charts == TRUE) {
    ...
    aWhereCharts::generateMultiplot(acc_precip_stdev
                                    ,rolling_avg_eppet
                                    ,max_temp_stdev 
                                    ,min_temp_stdev  
                                    ,cols = 2
